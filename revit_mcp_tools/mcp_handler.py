...
# 【本地部署模型兜底】清理本地部署模型思考内容
def clean_think_content(raw_text):
    # 空内容直接返回，不处理
    if not raw_text or not isinstance(raw_text, str):
        return raw_text
    
    end_tag = "</think>"
    tag_index = raw_text.lower().find(end_tag.lower())
    # 没找到则直接返回原文本
    if tag_index == -1:
        return raw_text.strip()
    
    final_text = raw_text[tag_index + len(end_tag):].strip()
...
def get_ai_mcp_command(messages, api_key):
    try:
        ...
        ai_output = clean_think_content(raw_result)
...
# 函数3：多轮工具调用引擎
# ==================================================
def multi_round_mcp_engine(user_query, api_key, max_rounds=10):
    """
    多轮MCP调用引擎
